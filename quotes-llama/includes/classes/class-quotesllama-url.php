...
class QuotesLlama_URL {
	...
	public function quotes_from_url( $nonce = '' ) {
		...
		// If First is not a string.
		if ( ! preg_match( '/^[a-z\s\-]+$/i', $first ) ) {
			return '<p>Invalid first name.</p>';
		}

		// If Last is not a string.
		if ( ! preg_match( '/^[a-z\s\-]+$/i', $last ) ) {
			return '<p>Invalid last name.</p>';
		}
		...
		if ( empty( $quotes_data ) ) {
			return '<p>' . $first . ' ' . $last . ' was not found.</p>';
		}
		...
		foreach ( $quotes_data as $quote ) {
			...
			if ( $show_name_image ) {
				...
				echo wp_kses_post(
					$ql->clickable(
						esc_html( trim( $quote->title_name . ' ' . $quote->first_name . ' ' . $quote->last_name ) )
					)
				);
				echo '</p>';
				echo '</span>';
